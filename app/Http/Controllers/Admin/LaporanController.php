...
class LaporanController extends Controller
{
    ...
    public function peminjaman(Request $request)
    {
        ...
        if ($request->has('export') && $request->export == 'excel') {
            return $this->exportPeminjamanToExcel($peminjamans);
        }

        return view('admin.laporan.peminjaman', compact('peminjamans'));
    }

    private function exportPeminjamanToExcel($peminjamans)
    {
        // Format data untuk export
        $data = $peminjamans->map(function ($peminjaman, $index) {
            $userName = $peminjaman->user 
                ? $peminjaman->user->name 
                : 'Data User Tidak Ditemukan';
            
            $barangName = $peminjaman->barang 
                ? $peminjaman->barang->nama_barang 
                : 'Data Barang Tidak Ditemukan';
            
            $tanggalPinjam = $peminjaman->tanggal_pinjam;
            
            $alasanPinjam = $peminjaman->alasan_pinjam;
        ...
        $fileName = 'Laporan_Peminjaman_' . \Carbon\Carbon::now()->format('d-m-Y_H-i-s') . '.xlsx';
